...
export default function VideoUploadScreen() {
  ...
  const uploadMutation = useMutation({
    mutationFn: async () => {
      if (!videoFile) throw new Error('Please select a video to upload');
      if (!title.trim()) throw new Error('Please enter a video title');
      if (!description.trim()) throw new Error('Please enter a video description');
          ...
          <View style={styles.uploadArea}>
            {videoFile ? (
              ...
              <TouchableOpacity style={styles.uploadBox} onPress={pickVideo}>
                ...
                <Text style={styles.uploadHint}>MP4, MOV, or WEBM (Max 500MB)</Text>
              </TouchableOpacity>
            )}
          </View>
        </View>
              ...
              >
                <View style={styles.visibilityLeft}>
                  ...
                  <View style={styles.visibilityInfo}>
                    <Text style={[styles.visibilityLabel, visibility === option.value && styles.visibilityLabelActive]}>{option.label}</Text>
                    <Text style={styles.visibilityDescription}>{option.description}</Text>
                  </View>
                </View>
                <View style={[styles.radioButton, visibility === option.value && styles.radioButtonActive]} />
              </TouchableOpacity>
            );
          })}
        </View>
...
const styles = StyleSheet.create({
  ...
  uploadButton: { backgroundColor: Colors.primary, paddingHorizontal: 20, paddingVertical: 8, borderRadius: 8, marginRight: 12, minWidth: 80, alignItems: 'center' },
  uploadButtonDisabled: { opacity: 0.6 },
  uploadButtonText: { color: Colors.text, fontSize: 15, fontWeight: '700' },
  content: { flex: 1 },
  scrollContent: { paddingBottom: 40 },
  section: { padding: 16, borderBottomWidth: 8, borderBottomColor: Colors.surface },
  ...
  uploadBox: { borderWidth: 2, borderColor: Colors.border, borderStyle: 'dashed', borderRadius: 12, paddingVertical: 48, alignItems: 'center', gap: 12 },
  uploadText: { fontSize: 16, fontWeight: '600', color: Colors.text, marginTop: 8 },
  uploadHint: { fontSize: 13, color: Colors.textSecondary },
  videoPreview: { backgroundColor: Colors.surface, borderRadius: 12, padding: 20, alignItems: 'center', gap: 16 },
  videoPreviewInfo: { alignItems: 'center', gap: 8 },
  videoFileName: { fontSize: 14, fontWeight: '600', color: Colors.text, maxWidth: '80%' },
  videoDuration: { fontSize: 14, fontWeight: '600', color: Colors.textSecondary },
  changeVideoButton: { paddingHorizontal: 20, paddingVertical: 10, borderRadius: 8, borderWidth: 1, borderColor: Colors.border },
  changeVideoButtonText: { fontSize: 14, fontWeight: '600', color: Colors.textSecondary },
  inputGroup: { marginBottom: 20 },
  label: { fontSize: 15, fontWeight: '600', color: Colors.text, marginBottom: 8, flexDirection: 'row', alignItems: 'center', gap: 6 },
  tagsContainer: { flexDirection: 'row', flexWrap: 'wrap', backgroundColor: Colors.surface, borderWidth: 1, borderColor: Colors.border, borderRadius: 10, padding: 8, minHeight: 50 },
  tagChip: { flexDirection: 'row', backgroundColor: Colors.primary, borderRadius: 15, paddingHorizontal: 10, paddingVertical: 4, alignItems: 'center', marginRight: 8, marginBottom: 8, gap: 4 },
  tagChipText: { fontSize: 14, color: Colors.text, fontWeight: '600' },
  tagChipRemove: { padding: 2 },
  tagInput: { flexGrow: 1, minHeight: 30, fontSize: 15, color: Colors.text, paddingHorizontal: 8, paddingVertical: 4 },
  ...
  visibilityOption: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', padding: 16, borderRadius: 12, borderWidth: 1, borderColor: Colors.border, backgroundColor: Colors.surface, marginBottom: 12 },
  visibilityOptionActive: { borderColor: Colors.primary, backgroundColor: Colors.surface },
  visibilityLeft: { flexDirection: 'row', alignItems: 'center', gap: 12, flex: 1 },
  visibilityInfo: { flex: 1 },
  visibilityLabel: { fontSize: 15, fontWeight: '600', color: Colors.text, marginBottom: 2 },
  visibilityLabelActive: { color: Colors.primary },
  visibilityDescription: { fontSize: 12, color: Colors.textSecondary },
  radioButton: { width: 20, height: 20, borderRadius: 10, borderWidth: 2, borderColor: Colors.border },
  radioButtonActive: { borderColor: Colors.primary, backgroundColor: Colors.primary },
